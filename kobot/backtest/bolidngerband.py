...
def SetRiskCoinCount(now_price, stop_loss_price, money_balance, risk):
...
def BackTestBollingerBand(movingAverage, k, risk):  # 이동평균선, 계수(승수)
    ...
    for idx, (close, upper, lower, stop_loss, index) in enumerate(
        ...
        if lower_flag and (close > lower) and (idx > 0):
            lower_flag = False
            now_coin = SetRiskCoinCount(close, stop_loss, money_balance_list[idx - 1], risk)
    ...
    print("##########################")
    # print(qs.stats.sharpe(df['profit']))
    htmlname = str(uuid.uuid4())+ ".html"
    filename = str(STATICFILES_DIRS)[2:-3] + "/" + htmlname
    print("???"+filename)
