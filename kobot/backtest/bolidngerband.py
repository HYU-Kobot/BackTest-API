...
def backTestBollingerBand(df, market, upper_moving_average, lower_moving_average, upper_k, lower_k, risk, timeFrame):
    ...
    df['stop_loss_price'] = df['close'] * 0.95
    ...
    for idx, (close, upper, lower, stop_loss, index) in enumerate(
        ...
        for loss_dict in stop_list[:]:
            if loss_dict['stop_loss_price'] > close:
                ...
                order_list.append({'category': 'SELL', 'market': market, 'amount': loss_dict['coin_count'],
                                   'trade_date': index, 'price' : close})
                loss_flag = True
            if loss_flag:
                break
        ...
        if lower_flag and (close > lower) and (idx > 0):
            ...
            order_list.append({'category': 'BUY', 'market': market, 'amount': now_coin, 'trade_date': index, 'price' : close})
        ...
        if upper_flag and (close < upper) and (coin_count[idx - 1] > 0) and (idx > 0):
            ...
            stop_list = []
            order_list.append(
                {'category': 'SELL', 'market': market, 'amount': coin_count[idx - 1], 'trade_date': index, 'price' : close})
